...
curl --insecure -sS -X GET $URL -H "accept: application/json" -H "x-auth-token: 460d3a3f-1d99-4c00-a3ff-9e7c81800eae" | jq -r '.overview.ram' > unmsrawoutput.txt
...
if [[ $result -gt $2 ]]
then
echo "CRITICAL - $devicename has high RAM usage."
 exit 3

else
echo "OK - $devicename RAM usage is over threshold."
 exit 0
